...
- **Compatibility builtin routing** — `DATE_FORMAT` → `DATE_FORMAT_MYSQL`,
  `TO_CHAR` → `DATE_FORMAT_PG`, `TRUNCATE` → `TRUNCATE_PRESTO`, etc.
  (real ClickZetta engine functions).
...
- **ClickHouse function mappings** — `FROMUNIXTIMESTAMP64MILLI`,
  `toDateTime`, `visitParamExtract*`, and friends.
