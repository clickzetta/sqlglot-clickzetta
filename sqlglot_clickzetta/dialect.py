...
from sqlglot import exp, time, transforms
...
# v23 sqlglot kept this table in sqlglot.dialects.hive with (func, multiplier)
# values; upstream v30 moved it to sqlglot.parsers.tsql and reshaped it to
# unit -> canonical unit. ClickZetta's expansion semantics need the old shape,
# so the table lives here now.
DATE_DELTA_INTERVAL = {
    "YEAR": ("ADD_MONTHS", 12),
    "MONTH": ("ADD_MONTHS", 1),
...
def _grouping_sql(self: ClickZetta.Generator, e: exp.Grouping) -> str:
    # ClickZetta exposes GROUPING_ID, not GROUPING
    return self.func("GROUPING_ID", *e.expressions)


def _parse_datetime_sql(self: ClickZetta.Generator, e: exp.ParseDatetime) -> str:
    return self.func("TO_TIMESTAMP", e.this, e.args.get("format"))


def _dayofweek_sql(self: ClickZetta.Generator, e: exp.DayOfWeek) -> str:
    # ClickZetta's DAYOFWEEK takes a DATE, not a timestamp — always emit the
    # TO_DATE wrap explicitly (inherited _to_date_sql unwraps TsOrDsToDate
    # when its parent is a ts-or-ds expression like DayOfWeek).
    this = e.this
    inner = this.this if isinstance(this, exp.TsOrDsToDate) else this
    return self.func("DAYOFWEEK", self.func("TO_DATE", inner))


def _yearofweek_sql(self: ClickZetta.Generator, e: exp.YearOfWeek) -> str:
...
def _split_sql(self: ClickZetta.Generator, e: exp.Split) -> str:
    # v23's Spark SPLIT generation quoted the delimiter without a closing \\E;
    # ClickZetta output keeps that shape.
    return self.func("SPLIT", e.this, f"CONCAT('\\\\Q', {self.sql(e.expression)})")


def _parse_json(self: ClickZetta.Generator, e: exp.ParseJSON) -> str:
    if isinstance(e.this, exp.Literal) and e.this.is_string:
        return f"JSON '{e.this.this}'"
    return self.func("PARSE_JSON", e.this)


# Doris AGGREGATE KEY table property (https://doris.apache.org/zh-CN/docs/4.x/table-design/data-model/aggregate).
# Upstream sqlglot has no node for it; we parse it (compat) and drop it at generation,
# since ClickZetta has no aggregate table model.
...
class ClickZetta(Spark):
    ...
    TIME_MAPPING = {
        ...
        "h": "%-I",    # hour 1-12 without leading zero
    ...
    class Parser(Spark.Parser):
        FUNCTIONS = {
            **Spark.Parser.FUNCTIONS,
            # Store the format literal verbatim: v30's Spark builder
            # strftime-izes it at parse time, which loses dialect tokens the
            # ClickZetta generator wants to emit back unchanged (e.g. SSSxxx).
            # The first argument keeps v30's TimeStrToTime wrap, which the
            # generator unwraps into CAST(... AS TIMESTAMP).
            "DATE_FORMAT": lambda args: exp.TimeToStr(
                this=exp.TimeStrToTime(this=seq_get(args, 0)), format=seq_get(args, 1)
            ),
        }
    ...
    class Generator(Spark.Generator):
        ...
        TYPE_MAPPING = {
            **Spark.Generator.TYPE_MAPPING,
            ...
            exp.DataType.Type.SMALLSERIAL: "INT",
        ...
        PROPERTIES_LOCATION[AggregateKeyProperty] = exp.Properties.Location.POST_SCHEMA
        ...
        TRANSFORMS = {
            **Spark.Generator.TRANSFORMS,
            ...
            AggregateKeyProperty: lambda self, e: "",
            ...
            exp.TimeToStr: _time_to_str_sql,
            exp.Split: _split_sql,
