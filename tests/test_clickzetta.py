...
AUTO PARTITION BY LIST (col1, col2)()""",
        ...
        # Diverged from fork: upstream v30 parses doris date_trunc(expr, unit)
        # correctly; the fork emitted the arguments swapped (unit='K0').
        self.validate_all(
            "CREATE TABLE test (k0 DATE) PARTITIONED BY (DATE_TRUNC('YEAR', k0))",
            read={
...
(PARTITION p1 VALUES LESS THAN ('2024-01-01'))""",
        ...
        self.validate_all(
            # Diverged from fork: correct date_trunc(expr, unit) argument order.
            "CREATE TABLE test (dt TIMESTAMP) PARTITIONED BY (DATE_TRUNC('DAY', dt))",
            read={
